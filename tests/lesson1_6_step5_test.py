...
class TestSearhOfElementByLink:
    ...
    def test_search_of_element_by_link(self, browser):
        try:
            main_page = BasePage(
                browser, "http://suninjuly.github.io/find_link_text.html"
            )
            main_page.open()
            locators = LocatorsLesson1_6_step5
            person_info = next(generated_person())
            main_page.find_element_and_click(
                (By.LINK_TEXT, str(math.ceil(math.pow(math.pi, math.e) * 10000)))
            )
            main_page.find_element_and_send_keys(
                locators.FIRST_NAME, person_info.firstname
            )
            main_page.find_element_and_send_keys(
                locators.LAST_NAME, person_info.lastname
            )
